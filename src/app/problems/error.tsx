...
export default function ProblemsErrorFallback({ error, reset }: ErrorProps) {
  useEffect(() => {
    if (process.env.NODE_ENV !== 'production') {
      console.error('[Problems Error Boundary caught]', error);
    }
  }, [error]);
  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 text-center bg-[#faf9f5] border border-[#e6dfd8] rounded-md max-w-md mx-auto my-12 shadow-sm animate-in fade-in duration-300">
      ...
      <h2 className="text-xl font-bold font-serif text-[#141413] mb-2">Something went wrong</h2>
...
export class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  public render() {
    if (this.state.hasError) {
      return (
        <ProblemsErrorFallback 
          error={this.state.error || new globalThis.Error('Runtime error in component island')} 
